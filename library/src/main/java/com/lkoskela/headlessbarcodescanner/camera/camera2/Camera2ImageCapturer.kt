...
/**
 * An [ImageCapturer] implementation using Camera2 for capturing the images.
 *
 * Heavily based on https://github.com/googlesamples/android-Camera2Basic sample code.
 */
internal class Camera2ImageCapturer(private val ctx: Context, private val camOutputSize: Size) : ImageCapturer {
    ...
    private fun createImageReader(): ImageReader {
        ...
        return SUPPORTED_IMAGE_FORMATS
                ...
                ?.let { ImageReader.newInstance(
                            camOutputSize.width, camOutputSize.height, it, 2) }
                ?.also {
    ...
    private fun camSupportsImageFormat(cam: CameraHolder, imageFormat: Int): Boolean {
        return cam.getSupportedSizesForFormat(imageFormat)?.contains(camOutputSize) ?: false
    }

    companion object {
        private const val TAG = "Cam2ImageReader"

        val CAM_OUTPUT_SIZE_SMALL = Size(320, 240)
        val CAM_OUTPUT_SIZE_MEDIUM = Size(640, 480)
        val CAM_OUTPUT_SIZE_LARGE = Size(1280, 720)
